        ...
        implements SeekingIterator<Slice, Slice>
{
    ...
    public void seek(Slice targetKey)
    {
        ...
        while (left < right) {
            ...
            Slice nextKey = nextEntry.getKey();

            //if (comparator.compare(nextEntry.getKey(), targetKey) < 0) {
            //if (comparator.compare(nextKey.copySlice(), targetKey.copySlice()) < 0) {
            if (comparator.compare(nextKey, targetKey) < 0) {
        ...
        for (seekToRestartPosition(left); nextEntry != null; next()) {
            Slice Key = peek().getKey();
            //int compareResult = comparator.compare(Key.copySlice(), targetKey.copySlice());
            int compareResult = comparator.compare(Key, targetKey);
            if (compareResult >= 0) {            
                break;
            }
        }
